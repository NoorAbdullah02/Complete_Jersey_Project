...
export default function OrderForm({ onSubmit, onPriceChange }) {
    ...
    const [customer, setCustomer] = useState({
        name: '', mobileNumber: '', email: '', notes: '', department: 'Department of ICE', batch: '',
    ...
    const [items, setItems] = useState([
        { id: 'init-' + Date.now(), jerseyNumber: '', jerseyName: '', size: '', collarType: 'polo', sleeveType: 'half', price: 360 }
    ]);
    ...
    const addItem = () => {
        setItems(prev => [
            ...prev,
            { id: `item-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, jerseyNumber: '', jerseyName: '', size: '', collarType: 'polo', sleeveType: 'half', price: 360 }
        ]);
    };
    ...
    const validateAll = () => {
        const newErrors = {};
        ['name', 'mobileNumber', 'email', 'batch'].forEach(f => {
            ...
            {options.map((opt) => (
                <div key={opt.value}
                    ...
                    style={{
                        ...
                        textAlign: 'center',
                        padding: '14px',
                        background: value === opt.value ? 'rgba(0, 242, 254, 0.8)' : 'transparent',
                        color: '#ffffff',
                        borderRadius: '12px',
                        cursor: 'pointer',
                        fontWeight: '600',
                        fontSize: '0.9rem',
                        transition: 'all 0.3s ease',
                        boxShadow: value === opt.value ? '0 0 20px rgba(0, 242, 254, 0.4)' : 'none',
    ...
    return (
        <section ref={formRef} className="order-section" style={{ background: 'transparent', padding: '60px 0', position: 'relative', zIndex: 10 }}>
            <h2 className="section-title text-center mb-5" style={{
                ...
                DEPARTMENT <span style={{ color: "var(--primary)" }}>OF ICE</span>
            </h2>
            ...
            <form id="jersey-order-form" className="order-form container" noValidate onSubmit={handleFormSubmit}>
                ...
                <div style={glassCardStyle} className="mb-5 glass-card">
                    ...
                    <h4 className="mb-5" style={{ ...headerStyle, fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '15px' }}>
                        <i className="fas fa-user-astronaut" style={{ color: 'var(--primary)' }}></i> YOUR DETAILS
                    </h4>
                    ...
                    <div className="row g-4">
                        ...
                        <div className="col-lg-6">
                            ...
                            <input type="text" className={`form-control-custom ${fieldClass('mobileNumber')}`}
                                ...
                                onBlur={(e) => handleBlur('mobileNumber', e.target.value)} required />
                        ...
                        <div className="col-lg-6">
                            ...
                            <div style={{ ...inputStyle, background: 'rgba(255,255,255,0.05)', color: 'var(--primary)', borderColor: 'transparent', cursor: 'default', display: 'flex', alignItems: 'center' }}>
                                <i className="fas fa-shield-alt me-2"></i> Department of ICE
                            </div>
                        </div>
                        <div className="col-lg-6">
                            <label style={labelStyle}>Batch / Role</label>
                            <select style={{ ...inputStyle, cursor: 'pointer' }}
                                value={['Teacher', 'Lab Assistant', 'Alumni'].includes(customer.batch) ? 'Others' : customer.batch}
                                onChange={(e) => handleCustomerChange('batch', e.target.value === 'Others' ? 'Others' : e.target.value)}>
                                <option value="" style={{ color: '#aaa' }}>SELECT BATCH</option>
                                {batches.map(b => <option key={b} value={b} style={{ color: '#000' }}>{b}</option>)}
                            </select>
                            {(customer.batch === 'Others' || ['Teacher', 'Lab Assistant', 'Alumni'].includes(customer.batch)) && (
                                <div className="mt-3">
                                    <select style={{ ...inputStyle, padding: '14px', background: 'rgba(255,255,255,0.05)' }}
                                        value={['Teacher', 'Lab Assistant', 'Alumni'].includes(customer.batch) ? customer.batch : ''}
                                        onChange={(e) => handleCustomerChange('batch', e.target.value)}>
                ...
                <div className="items-container">
                    {items.map((item, index) => (
                        <div key={item.id} style={glassCardStyle} className="mb-5 glass-card">
                            <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '4px', background: 'linear-gradient(90deg, var(--secondary), var(--accent))' }}></div>
                            ...
                            <div className="row g-4">
                                ...
                                <div className="col-12">
                                    <label style={labelStyle}>Name on Jersey</label>
                                    <input type="text"
                                        placeholder="E.G. HASAN" style={{ ...inputStyle, textTransform: 'uppercase', fontSize: '1.2rem', fontWeight: '600', letterSpacing: '2px', textAlign: 'center' }}
                                        value={item.jerseyName} onChange={(e) => handleItemChange(item.id, 'jerseyName', e.target.value)}
                                        
                                        onBlur={(e) => handleBlur('jerseyName', e.target.value, item.id)} />
                                ...
                                <div className="col-lg-6">
                                    ...
                                    <div className="position-relative">
                                        <input type="text"
                                            ...
                                            onBlur={(e) => handleBlur('jerseyNumber', e.target.value, item.id)} />
                                    </div>
                                ...
                                <div className="col-12 mt-3">
                                    <label style={labelStyle}>Collar Type</label>
                                    <SegmentedControl
                                ...
                                <div className="col-12">
                                    <label style={labelStyle}>Sleeve Type</label>
                                    <SegmentedControl
                                        ...
                                        {['S', 'M', 'L', 'XL', 'XXL', 'XXXL'].map(s => (
                                            <div key={s}
                                                ...
                                                style={{
                                                    ...
                                                    background: item.size === s ? 'var(--primary)' : 'rgba(255,255,255,0.05)',
                                                    border: item.size === s ? '1px solid var(--primary)' : '1px solid rgba(255,255,255,0.1)',
                                                    borderRadius: '12px',
                                                    color: '#ffffff',
                                                    fontWeight: '700',
                                                    fontSize: '1rem',
                                                    ...
                                                    boxShadow: item.size === s ? '0 0 20px rgba(0, 242, 254, 0.5)' : 'none',
                                ...
                                <div className="col-12 mt-5 pt-4 d-flex justify-content-between align-items-center" style={{ borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                                    <span style={{ color: 'var(--primary)', fontSize: '0.9rem', fontWeight: '500', letterSpacing: '1px' }}>TOTAL</span>
                ...
                <div className="submit-container pb-5 text-center">
                    ...
                    <button type="submit"
                        style={{
                            ...
                            fontWeight: '800',
                            fontFamily: "'Orbitron', sans-serif",
                            boxShadow: '0 10px 40px rgba(0, 242, 254, 0.5)',
