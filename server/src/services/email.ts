...
export function orderConfirmationText(order: any): string {
  const itemsText = (order.items || []).map((item: any) =>
    `- Jersey #${item.jerseyNumber || item.jersey_number} (${item.size}) | Name: ${item.jerseyName || item.jersey_name || 'N/A'}`
  ).join('\n');
...
export function adminNotificationHtml(order: any): string {
  const itemsHtml = (order.items || []).map((item: any) =>
    `<li><strong>#${item.jerseyNumber || item.jersey_number}</strong> (${item.size}) - ${item.jerseyName || item.jersey_name || 'N/A'}</li>`
  ).join('');
...
export function paymentConfirmationHtml(order: any): string {
  const itemsHtml = (order.items || []).map((item: any) =>
    `<li style="margin-bottom: 5px;"><strong>#${item.jerseyNumber || item.jersey_number}</strong> (${item.size})</li>`
  ).join('');
...
export function adminVerificationHtml(name: string, verifyUrl: string): string {
  return `
