...
router.post('/', async (req, res) => {
    try {
        ...
        const { db, client } = await (0, db_1.getDb)();
        try {
            const result = await client.query('INSERT INTO expenses (description, amount, category, date, created_by) VALUES ($1, $2, $3, $4, $5) RETURNING *', [description.trim(), parseFloat(amount), category || 'general', date || null, req.user?.username || null]);
    ...
    try {
        ...
        try {
            const result = await client.query(`UPDATE expenses 
                 ...
                 WHERE id = $5 RETURNING *`, [description?.trim(), amount ? parseFloat(amount) : null, category, date, req.params.id]);
