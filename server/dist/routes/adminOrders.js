...
router.get('/orders', async (req, res) => {
    ...
    `;
        ...
        const { db, client } = await (0, index_1.getDb)();
        try {
    ...
    try {
      ...
      `, [req.params.id]);
            ...
            if (status === 'done' && order.status !== 'done') {
                await (0, email_1.sendEmail)(order.email, 'Payment Confirmed - ICE Jersey Order', (0, email_1.paymentConfirmationHtml)(order), `Your payment has been confirmed. Order ID: ICE-${order.id.toString().padStart(3, '0')}`);
            }
    ...
    try {
      ...
      `);
            ...
            const emailPromises = users.map(async (user) => {
                try {
                    const success = await (0, email_1.sendEmail)(user.email, 'Jersey Collection Ready! - ICE Department', `
